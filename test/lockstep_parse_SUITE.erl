...
chunked_partial_parse(Config) ->
    Table = ?config(tid, Config),
    CallbackState = {chunked_partial_parse, Table},
    Data1 = get_message(),
    Data2 = get_message(),
    ...
    {ok, CallbackState, Rest} = chunked_parser:parse_msgs(to_bin(PartM1), lockstep_parse_callback,
                                                      CallbackState),
    {ok, end_of_stream, CallbackState} = chunked_parser:parse_msgs(<<Rest/binary, (to_bin(PartM2))/binary>>,
                                                    lockstep_parse_callback, CallbackState),
